...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
	closestGhostDistances = util.Counter()
	for dist in livingGhostPositionDistributions:
	    [tempMax,bestPos] = [0,None]
	    for (pos, prob) in dist.items():
		if tempMax <= prob: [tempMax, bestPos] = [prob, pos]
	    closestGhostDistances[bestPos] = bestPos

	[minDis, bestMove] = [9999,None]
	for bestPos in closestGhostDistances:
	    [minActionDis,bestAction] = [9999,None]
	    for action in legal:
		pos = Actions.getSuccessor(pacmanPosition,action)
		temp = self.distancer.getDistance(bestPos,Actions.getSuccessor(pacmanPosition,action))
                if temp < minActionDis: [minActionDis,bestAction] = [temp,action]
            if minActionDis < minDis: [minDis, bestMove] = [minActionDis,bestAction]
	return bestMove
