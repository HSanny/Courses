...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
	self.particles = list()
	averageParticles = self.numParticles/len(self.legalPositions)
	...
	if(noisyDistance != None):
		...
		if emissionModel[trueDistance] > 0:
	            newDis[p] = emissionModel[trueDistance]*beliefDistribution[p]
	    if newDis.totalCount() != 0:
		items = newDis.items()
           	distribution = [i[1] for i in items]
                values = [i[0] for i in items]
	        allPossible = util.nSample(distribution,values,self.numParticles)
                #for i in range(self.numParticles): allPossible.append(util.sample(newDis))
	else:
	    allPossible = [self.getJailPosition()]*self.numParticles
	    #for i in range(self.numParticles): allPossible.append(jailPosition)
        self.particles = allPossible
        #print "In Observe: ",time.time()-startTime
        "*** END YOUR CODE HERE ***"
	...
	for p in self.legalPositions:
	    newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
	    for newPos, prob in newPosDist.items():
		newDis[newPos] = newDis[newPos] + prob*beliefDistribution[p]
	#print "Mid time: ",time.time()-startTime
        if newDis.totalCount() != 0:
	    items = newDis.items()
            distribution = [i[1] for i in items]
            values = [i[0] for i in items]
	    allPossible = util.nSample(distribution,values,self.numParticles)
                #for i in range(self.numParticles): allPossible.append(util.sample(newDis))
        self.particles = allPossible
	#print "In elapseTime: ",time.time()-startTime,"|",self.numParticles
	...
	particleWeights = util.Counter()
        for pos in self.legalPositions:
	    particleWeights[pos] = self.particles.count(pos)
	particleWeights[self.getJailPosition()] = self.particles.count(self.getJailPosition())
...
class JointParticleFilter:
	...
	for purple in perMutationPos:
	    print self.particles.count(purple)
	...
	"""for i in range(len(noisyDistances)):
	    print noisyDistances[i] == None," : ",noisyDistances[i]"""


	"""beliefDistributions = self.getBeliefDistribution()
	uniqueSet = set()
	for particle in self.particles:
	    if particle not in uniqueSet:
	       print particle," | ", beliefDistributions[particle]
	    uniqueSet.add(particle)
	time.sleep(500)	"""

	weights = util.Counter()
	for particle in self.particles:
	    partWeight = 1
	    #print particle," | ",particle[0]," | ",pacmanPosition
		...
		if noisyDistances[ghostIndex] != None:
		    trueDistance = util.manhattanDistance(particle[ghostIndex], pacmanPosition)
          	    partWeight *= emissionModels[ghostIndex][trueDistance]
	    weights[particle] += partWeight
	    #print particle," | " ,partWeight, " | ", weights[particle]
	#time.sleep(500)
	weights.normalize()
	...
	if weights.totalCount() == 0:
	    ...
	    #for i in range(self.numParticles): self.particles.append(util.sample(weights))
	    items = weights.items()
            distribution = [i[1] for i in items]
            values = [i[0] for i in items]
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        particleWeights = util.Counter()
	for partiTuple in self.particles:
	    particleWeights[partiTuple] += 1
