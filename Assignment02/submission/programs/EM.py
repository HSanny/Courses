...
def randomInitialise(patterns, gaussians):
    ...
    for i in range(0, numOfComponents):
        ## randomise the mean of gaussians as samples of multivariate normal distribution
        ## whose center is the mean of all data objects.
        ...
        ## initialise covariance matrix to identity matrix
...
def expectation(patterns, gaussians, piList):
    '''
        Apply E-step of EM algorithm. 
        compute the responsibilities of each components to each input patterns.
        return a 2-d array containing n * d responsibilities.
    '''
...
def maximisation(patterns, responsibilities, gaussians):
    '''
        Apply M-step of EM algorithm.
        compute the mu, sigma and pi update.
        return renewed gaussian components and marginal probability of latent variable.
    '''
    numOfComponents = len(gaussians)
...
def getCriteria( patterns, gaussians, piList, responsibilities):
    '''
        compute loglikelihood and expected loglikelihood of certain "state".
    '''
    ## verify arguments
    ...
    for i in range(0, numOfPatterns):
        ...
        for j in range(0, numOfComponents):
            ...
            if tempJointProbability == 0:
                tempExpectedLikelihood += responsibilities[i, j] * np.log(1e-6)
            else:
