  ...
  for(i=0; i<nvectors; i++)
    {
      ...
      if (!testing)
	if (((best/second <= second/best) && (best/second > 0.01))
	    || ((best/second > second/best) && (second/best > 0.01)))
  ...
  if (!labeling)
    {
      ...
      if (strcasecmp(labelvectors[units[besti][bestj].labels[0]].label,
		     vectors[inpv].label))
	{
	  misclassified++;
	  bestmisclassified=1;
...
compute_labels(int inpv)
{
  ...
  for(i=0; i<nx; i++)
    for(j=0; j<ny; j++)
      {
	units[i][j].value = distance(labelvectors[inpv].comp,
				     units[i][j].comp,dim);
	if (units[i][j].value < units[i][j].bestvalue){
	  units[i][j].bestvalue = units[i][j].value;
	  units[i][j].labels[0] = inpv;
...
modify_weights(int nc, float alpha, int inpv)
{
  register int k;
  if (inpv==21)
    printf("*(%d %d) %d %f, (%d %d) %d %f: %f %f\n",
	   besti, bestj, bestmisclassified, best,
	   secondi, secondj, secondmisclassified, second,
...
init_params(char *arg1, char *arg2)
{
  int c;
...
read_params(FILE *fp)
{
  ...
  fscanf(fp,"%s", labelvectorfile); fgets(s,99,fp);
...
read_inputs()
{
  ...
  fp=fopen(labelvectorfile,"r");

  /* read labelfilename and vector dimension */
  fscanf(fp,"%s", labellabelfile); fgets(s,99,fp);
  fscanf(fp,"%d", &dim); fgets(s,99,fp);

  /* read the labels */
  fp2=fopen(labellabelfile,"r");
  for(i=0; i<MAXVECTORS && fscanf(fp2,"%s", labelvectors[i].label)!=EOF; i++);
  nlabelvectors=i;
  fclose(fp2);

  /* read the vectors */
  for(i=0; i<nlabelvectors; i++)
    for(j=0; j<dim; j++)
      fscanf(fp,"%f",&labelvectors[i].comp[j]);
  fclose(fp);
}
