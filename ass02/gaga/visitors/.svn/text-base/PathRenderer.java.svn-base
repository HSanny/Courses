...
public class PathRenderer extends Java2DRenderer implements PathVisitor {
    // switch for outputing test string to console
    static final boolean PRINTING = false;

    private SvgElement e;

    private Point2D firstPoint = null; // used for closepath
    private Point2D currentPosition; // indicate state of path visiting

    private boolean drawable = false; // indicate whether current visiting lead to drawing
    public boolean isDrawable () { return this.drawable; }

    private boolean isLastInstructionMoveTo = false;
    // constructor. note that PathRenderer extends java2DRenderer
    // such that many drawing method can be invoked.
    ...
    public void visitLineTo(LineTo lineto) {
        ...
        if (command.compareTo("L") == 0){
            ...
            // horizental lines with absolute movement
            x2 = lineto.getAbscissa() ;     
            y2 = y1 ;
        } else if (command.compareTo("h") == 0) {
            // horizental lines with relative movement
            x2 = lineto.getAbscissa() + x1;     
            y2 = y1 ;
        } else if (command.compareTo("V") == 0) {
            // vertical lines with absolute movement
            x2 = x1 ;     
            y2 = lineto.getOrdinate() ;
        } else if (command.compareTo("v") == 0) {
            // vertical lines with relative movement
            x2 = x1 ;     
            y2 = lineto.getOrdinate() + y1;
        }   
 
        // renew current position of path visitor
        this.currentPosition = new Point2D (x2, y2); 
        if (this.isDrawable() ) {
            // but the move command is in its first occurrence
            shape = new Line2D.Double(x1*scale,y1*scale,x2*scale,y2*scale);
        ...
        // check if renewed position is the first specified position
        this.checkFirstPoint(x2, y2);
    ...
    public void visitClosePath(ClosePath closepath){
        ...
        if ( this.firstPoint != null ) {
            ...
            double y2 = this.firstPoint.getOrdinate();
            shape = new Line2D.Double(x1*scale,y1*scale,x2*scale,y2*scale);
            checkStyles(e);
            drawBorder(e, gbi, shape);
            gbi.fill(shape);
    ...
    public void visitArc(Arc arc) {
        ...
        this.currentPosition = new Point2D (x2, y2); 
        if (this.isDrawable() ) {
            double x3 = (x1 + x2) /2.0 - 10*radiusX * Math.sin(rotationX);
            double y3 = (y1 + y2) /2.0 - 10*radiusX * Math.cos(rotationX);
            // here we use QuadCurve2D to replace the role of arc
            //  this is a substitution with flaws, but better than rectangle
    ...
    public void visitCubicCurveTo(CubicCurveTo cct) {
        ...
        if (command.compareTo("C") == 0){
            ...
            // relative movement for cubic curive to
    ...
    public void visitQuadraticCurveTo(QuadraticCurveTo qct) {
        ...
        // draw method should be invoked in this case
        this.drawable = true; 

        // renew the current position of path visitor
