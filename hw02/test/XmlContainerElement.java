...
import comp2100.oops.strategies.*;
 ...
 * @version $Revision: 2005.8 $, $Date: 2005/04/12 01:14:21 $
 */
public class XmlContainerElement extends XmlElement {
    ...
    public Vector<XmlElement> children;
	...
	if (children.size() == 0) {
		...
		(children.elementAt(i)).prettyPrint(indent + 3, 
								  output);
	    output.write(StringOps.spaces(indent));
	    output.write("</" + name + ">\n");
	}
	...
	if (children != null) {
		...
		children.elementAt(i).accept(visitor);
	    }
	}
    }
}
